...
#import bevy_terrain::node NodeLookup,lookup_node
#import bevy_terrain::functions Fragment,FragmentInput,FragmentOutput,calculate_blend,calculate_normal
#import bevy_terrain::uniforms atlas_sampler,config,height_atlas,view_config,quadtree
...
fn fragment(input: FragmentInput) -> FragmentOutput {
    ...
    let lookup = lookup_node(blend.lod, input.local_position, config.lod_count, view_config.node_count, quadtree, config.leaf_node_size);
    var data = lookup_fragment_data(input, lookup, ddx, ddy);

    if blend.ratio < 1.0 {
        let lookup2 = lookup_node(blend.lod + 1u, input.local_position, config.lod_count, view_config.node_count, quadtree, config.leaf_node_size);
...
// height_atlas: texture_2d_array<f32>, atlas_sampler: sampler, coords: vec2<f32>, atlas_index: i32, atlas_lod: u32, ddx: vec2<f32>, ddy: vec2<f32>, height_size: f32, height: f32
// need to pass in height_size and height
...
fn process_fragment(input: FragmentInput, data: FragmentData) -> Fragment {
    let do_discard = input.local_position.x < 2.0 || input.local_position.x > f32(config.terrain_size) - 2.0 || input.local_position.y < 2.0 || input.local_position.y > f32(config.terrain_size) - 2.0;
